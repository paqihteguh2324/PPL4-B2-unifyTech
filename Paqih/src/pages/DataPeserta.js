...
function DataPeserta() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      //console.log(result.data.data.attributes.jabatan.data.attributes.nama_jabatan);
      const arr = result.data.data;
     var sum = arr.map((item) => item).filter((item) => item.attributes.role_job.data.attributes.rule === 'Peserta' || item.attributes.role_job.data.attributes.rule === 'Double' );
     setTodos(sum);
    };
    fetchData();
  }, []);
